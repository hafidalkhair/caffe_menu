...
@section('content')
    ...
    <div class="max-w-4xl mx-auto mb-8 flex flex-col md:flex-row md:items-center justify-between gap-4 fade-in-up">
        <div>
            ...
            <p class="text-sm text-gray-500 mt-1">Perbarui informasi menu yang sudah ada.</p>
        </div>
    ...
    <div class="max-w-4xl mx-auto bg-white rounded-2xl shadow-sm border border-gray-100 p-8 fade-in-up" style="animation-delay: 0.1s;">
        ...
        <form action="{{ route('admin.menus.update', $menu->id) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                {{-- Harga (DIPERBAIKI: Type Text & Javascript Format) --}}
                <div>
                    ...
                    <div class="relative group">
                        ...
                        {{-- PERUBAHAN: type="text" dan value menggunakan number_format agar ada titik saat load awal --}}
                    ...
                    <div class="flex items-start gap-6">
                        @if($menu->image)
                            <div class="flex-shrink-0">
                                <p class="text-xs text-gray-400 mb-2">Saat ini:</p>
                                <img src="{{ str_contains($menu->image, 'images/menus/') ? asset($menu->image) : asset('storage/' . $menu->image) }}"
                                     alt="Current Image"
                                     class="w-32 h-32 object-cover rounded-xl border-2 border-gray-100 shadow-sm">
                            </div>
                        @endif

                        <div class="flex-grow">
                            <label for="image" class="flex flex-col items-center justify-center w-full h-32 border-2 border-gray-300 border-dashed rounded-xl cursor-pointer bg-gray-50 hover:bg-gray-100 transition-colors group">
                                <div class="flex flex-col items-center justify-center pt-5 pb-6">
                                    <i class="fas fa-cloud-upload-alt text-3xl text-gray-400 group-hover:text-indigo-500 mb-2 transition-colors"></i>
                                    <p class="text-sm text-gray-500"><span class="font-semibold">Klik untuk ganti foto</span></p>
...
@push('scripts')
<script>
    // FUNGSI FORMAT RUPIAH (Otomatis Tambah Titik)
